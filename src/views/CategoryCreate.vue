    ...
    <div class="max-w-7xl mx-auto">
      ...
      <div class="grid lg:grid-cols-2 gap-8 mb-8">
        ...
        <div class="glass-card">
          ...
          <form @submit.prevent="submitForm" class="space-y-6">
            ...
            <!-- Price Calculator -->
            <div v-if="formData.precioNormal && formData.porcentajeXPersona" class="glass-preview-card">
              ...
              <div class="space-y-2">
                <div class="flex justify-between text-sm">
                  ...
                  <span class="text-white font-medium">{{ formatPrice(parseFloat(formData.precioNormal || 0)) }}</span>
                </div>
                ...
                <div class="flex items-start justify-between">
                  <div class="flex-1">
                    ...
                    <p class="text-white/70 text-sm mt-1">
                      {{ formData.capacidadMaxima || 0 }} persona{{ (formData.capacidadMaxima || 0) !== '1' ? 's' : '' }} máximo
                    </p>
                  </div>
      ...
      <div class="glass-card">
        ...
        <div v-else class="grid lg:grid-cols-2 xl:grid-cols-3 gap-4">
          <div v-for="categoria in categorias" :key="categoria.categoriaId"
            ...
            <div class="p-6">
              ...
              <div class="space-y-2 mb-4">
                <div class="flex justify-between items-center">
                  ...
                  <span class="text-white font-semibold">{{ formatPrice(categoria.precioNormal) }}</span>
                </div>
                <div class="flex justify-between items-center">
...
const calculateMaxPrice = () => {
  ...
  return formatPrice(CategoryService.calculatePriceWithPercentage(basePrice, percentage, extraPersons))
}

const calculateCategoryMaxPrice = (categoria) => {
  const extraPersons = Math.max(0, categoria.capacidadMaxima - 1)
  return CategoryService.calculatePriceWithPercentage(categoria.precioNormal, categoria.porcentajeXPersona, extraPersons)
}
...
const createCategory = async () => {
  try {
    ...
    if (USE_V1_API) {
      ...
      response = await CategoryService.legacyCreateCategory(categoryData)
    }
...
const updateCategory = async () => {
  try {
    ...
    let response
    if (USE_V1_API) {
      response = await CategoryService.updateCategory(formData.value.categoriaId, categoryData)
    } else {
      response = await CategoryService.legacyUpdateCategory(formData.value.categoriaId, categoryData)
    }
...
const deleteCategory = async (categoria) => {
  ...
  try {
    let response
    if (USE_V1_API) {
      response = await CategoryService.deleteCategory(categoria.categoriaId)
    } else {
      response = await CategoryService.legacyDeleteCategory(categoria.categoriaId)
    }
...
const fetchCategories = async () => {
  try {
    ...
    let response
    if (USE_V1_API) {
      response = await CategoryService.getCategories(institucionId)
    } else {
      response = await CategoryService.legacyGetCategories(institucionId)
    }
