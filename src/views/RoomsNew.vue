<template>
  <div class="min-h-screen bg-gradient-to-br from-surface-950 via-surface-900 to-surface-950 relative overflow-hidden">
    ...
    <div class="relative z-10 p-6">
      ...
      <!-- Loading state -->
      <div v-if="roomsStore.isLoading && !roomsStore.allRooms.length" class="flex items-center justify-center py-20">
          ...
          <div class="space-y-6">
            <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-6 shadow-2xl">
              ...
              <div :class="freeRoomsGridClasses">
                <template v-for="(room, index) in combinedFreeAndMaintenanceRooms" :key="room.habitacionId">
                  <!-- Separador visual para habitaciones en mantenimiento -->
                  <div v-if="isFirstMaintenanceRoom(room, index)" class="col-span-full flex items-center gap-3 py-2 my-2">
                    <div class="flex-1 h-px bg-gradient-to-r from-transparent via-yellow-400/30 to-transparent"></div>
                    <span class="text-yellow-400 text-xs font-medium px-2 py-1 bg-yellow-400/10 rounded-full border border-yellow-400/20">
                      En Mantenimiento
                    </span>
                    <div class="flex-1 h-px bg-gradient-to-r from-transparent via-yellow-400/30 to-transparent"></div>
                  </div>
                  ...
                  <RoomCard
                    ...
                    @click="room.estadoHabitacion === 'Mantenimiento' ? openMaintenanceRoomModal(room) : openFreeRoomModal(room)"
                  />
                </template>
              </div>
            </div>
          ...
          <div class="bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl overflow-hidden shadow-2xl">
            <div class="p-6 bg-gradient-to-r from-green-600/20 to-emerald-600/20 border-b border-green-500/30">
              <h3 class="text-white font-bold text-xl flex items-center gap-3">
                ...
                <span v-if="filteredMaintenanceRooms.length > 0" class="text-yellow-400 text-sm font-normal">
                  • {{ filteredMaintenanceRooms.length }} en mantenimiento
                </span>
              </h3>
            </div>
              ...
              <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 gap-4">
                <template v-for="(room, index) in combinedFreeAndMaintenanceRooms" :key="room.habitacionId">
                  ...
                  >
                    ...
                    <div class="flex-1">
                      <span class="text-white font-medium block">{{ roomUtils.getRoomNumber(room.nombreHabitacion) }}</span>
                      <span :class="[
                        'text-sm',
                        room.estadoHabitacion === 'Mantenimiento' ? 'text-yellow-400' : 'text-gray-400'
                      ]">
                        {{ roomUtils.getCategoryFromName(room.nombreHabitacion) }}
                        <span v-if="room.estadoHabitacion === 'Mantenimiento'" class="ml-2">• Mantenimiento</span>
                      </span>
                    </div>
                    <span :class="[
                      'text-sm font-bold',
                      room.estadoHabitacion === 'Mantenimiento' ? 'text-yellow-300' : 'text-green-300'
                    ]">${{ room.precio }}/h</span>
                  </div>
                </template>
              </div>
                ...
                >
                  ...
                  <div class="flex-1">
                    <div class="flex items-center gap-3 mb-1">
                      <span class="text-white font-medium">{{ roomUtils.getRoomNumber(room.nombreHabitacion) }}</span>
                      <span v-if="room.pedidosPendientes" class="text-red-400 animate-pulse">
                        <span class="material-symbols-outlined text-sm">notifications_active</span>
                      </span>
                    </div>
                    <div class="text-gray-400 text-sm">
                      {{ room.visita?.identificador || 'Sin cliente' }} • {{ roomUtils.getCategoryFromName(room.nombreHabitacion) }}
                    </div>
                  </div>
                  <div class="text-right">
                    <div class="text-sm font-medium" :class="roomUtils.getTimeTextColor(room)">{{ roomUtils.getTimeRemaining(room) }}</div>
...
import { useRoomWebSocket } from '../composables/rooms/useRoomWebSocket';
...
// Note: Auto-refresh is handled by WebSocket (SignalR) for real-time updates
...
onUnmounted(() => {
  // WebSocket cleanup is handled by useRoomWebSocket composable
});
</script>
