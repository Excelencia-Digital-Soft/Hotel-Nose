    ...
    <div class="glass-container mb-6 p-6 transform hover:scale-[1.02] transition-all duration-300">
      <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between space-y-4 lg:space-y-0">
        <div class="text-center lg:text-left">
          ...
          <p class="text-gray-300 text-lg">Control completo de consumos personales y administración 📊</p>
        </div>
    ...
    <div v-if="!hasAnyData && !loading" class="text-center py-16">
      <div class="glass-card max-w-md mx-auto p-8">
        <div class="mb-6">
          ...
          <p class="text-gray-300 mb-6">Comienza registrando tu primer consumo o consulta tu historial</p>
        </div>
        ...
        <div class="space-y-3">
          <button
            ...
            class="bg-gradient-to-r from-green-400 to-green-500 hover:from-green-500 hover:to-green-600 
                   text-white font-bold py-3 px-6 rounded-lg transform hover:scale-105 transition-all duration-300 w-full"
          >
            <i class="pi pi-plus mr-2"></i>
...
const hasAnyData = computed(() => {
  return myConsumption.value?.length > 0 || mySummary.value || myConsumptionByService.value?.length > 0
})
...
const exportConsumption = () => {
  exportToCSV(formattedMyConsumption.value, `mi_consumo_${dateFilter.value.startDate}_${dateFilter.value.endDate}.csv`)
}
...
const formatDateRange = () => {
  ...
  const start = new Date(dateFilter.value.startDate).toLocaleDateString('es-PE')
  const end = new Date(dateFilter.value.endDate).toLocaleDateString('es-PE')
