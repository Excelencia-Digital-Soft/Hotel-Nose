<template>
      ...
      <div>
        ...
        <div class="grid grid-cols-3 gap-4">
          ...
          >
            ...
            <button
              @click="confirmDeleteArticulo(articulo)"
            ...
            <button
              @click="confirmUpdateArticulo(articulo)"
    ...
    <div class="flex justify-center space-x-4">
      <button
        @click="deleteArticulo(articuloToDelete?.articuloId)"
        class="bg-red-500 text-white px-4 py-2 rounded hover:bg-red-700"
      >
        Eliminar
      </button>
      <button
        @click="cancelDeleteArticulo"
  ...
  const createArticulo = async () => {
    if (newArticuloName.value && newArticuloPrice.value) {
      try {
        // Make sure to format the URL correctly
        const response = await axiosClient.post(
          `/api/Articulos/CreateArticulo?nombre=${encodeURIComponent(newArticuloName.value)}&precio=${newArticuloPrice.value}`
        );
