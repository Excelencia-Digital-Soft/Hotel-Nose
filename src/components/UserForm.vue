<template>
  ...
  <div class="w-1/3 h-auto fixed top-40 flex flex-col justify-evenly items-center p-8 pb-16 rounded-3xl self-start mt-2 border-x-8 border-y-2 border-accent-500 bg-neutral-800">
    ...
    <form class="flex flex-col items-center justify-center w-full" @submit.prevent="saveUsuario">
      ...
      <select v-model="form.rolId" class="w-full p-2 border rounded mb-4" required>
        <option v-for="rol in roles" :key="rol.rolId" :value="rol.rolId">{{ rol.nombreRol }}</option>
      </select>
      
      <div class="flex w-full justify-center mt-4">
        <button type="button" @click="$emit('close')" class="absolute top-2 right-2 w-10 h-10 btn-danger text-white rounded-full  font-bold">x</button>
        <button type="button" @click="$emit('close')" class="w-full py-2  btn-danger text-white rounded mr-2">Cancelar</button>
        <button type="submit" class="w-full py-2  btn-secondary text-white rounded">Guardar</button>
      </div>
    </form>
  </div>
</Transition>
...
watch(() => props.usuario, (newUsuario) => {
  if (newUsuario) {
    form.value = { nombreUsuario: newUsuario.nombreUsuario, rolId: newUsuario.rolId };
  } else {
