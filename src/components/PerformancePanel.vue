  ...
  <div class="performance-panel glass-card p-4 w-80">
    ...
    <div v-show="!isMinimized">
      ...
      <div class="flex items-center gap-2 mb-2">
        <div :class="['w-3 h-3 rounded-full', levelColors[performanceLevel]]" />
    ...
    <div class="mb-4" v-if="deviceInfo.memory || deviceInfo.cores">
      ...
      <div class="text-gray-300 text-xs space-y-1">
        <div v-if="deviceInfo.memory">RAM: {{ deviceInfo.memory }}GB</div>
        <div v-if="deviceInfo.cores">CPU: {{ deviceInfo.cores }} cores</div>
        <div v-if="deviceInfo.connection">Red: {{ deviceInfo.connection.effectiveType }}</div>
      </div>
    </div>
...
<script setup>
  ...
  const performanceTips = {
    ...
    minimal:
      'Efectos deshabilitados. Máxima compatibilidad y velocidad para dispositivos muy antiguos.',
  }
</script>
