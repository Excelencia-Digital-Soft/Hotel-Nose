  ...
  >
    ...
    <div class="flex items-center justify-between mb-4">
      <div class="flex items-center">
        <div
          class="bg-gradient-to-r from-primary-400 to-secondary-500 p-3 rounded-full mr-3 group-hover:scale-110 transition-transform"
        >
          <i class="pi pi-calculator text-white text-lg"></i>
        </div>
        <div>
          <h4 class="text-lg font-bold text-white mb-1">🧮 Cierre #{{ cierre.cierreId }}</h4>
    ...
    <div class="grid grid-cols-2 gap-4 mb-4">
      ...
      <div class="bg-white/5 rounded-lg p-3 border border-white/10">
        ...
        <p class="text-white font-semibold text-sm">
          ${{ formatCurrency(cierre.montoInicialCaja) }}
        </p>
      </div>
      ...
      <div class="bg-white/5 rounded-lg p-3 border border-white/10">
        ...
        <p class="text-white font-semibold text-sm">
          ${{ formatCurrency(cierre.totalIngresosEfectivo) }}
        </p>
      </div>
    </div>
    ...
    <div class="grid grid-cols-3 gap-2 mb-4">
      <!-- Cash -->
      <div
        class="bg-gradient-to-br from-emerald-500/10 to-emerald-600/5 rounded-lg p-2 border border-emerald-500/20"
      >
        <div class="flex items-center mb-1">
      ...
      <!-- Card -->
      <div
        class="bg-gradient-to-br from-blue-500/10 to-blue-600/5 rounded-lg p-2 border border-blue-500/20"
      >
        <div class="flex items-center mb-1">
        ...
        <p class="text-blue-400 font-semibold text-xs">
          ${{ formatCurrency(cierre.totalIngresosTarjeta) }}
        </p>
      </div>

      <!-- BillVirt -->
      <div
        class="bg-gradient-to-br from-purple-500/10 to-purple-600/5 rounded-lg p-2 border border-purple-500/20"
      >
        <div class="flex items-center mb-1">
          <i class="pi pi-mobile text-purple-400 text-xs mr-1"></i>
          <span class="text-gray-300 text-xs">Virtual</span>
        </div>
        <p class="text-purple-400 font-semibold text-xs">
          ${{ formatCurrency(cierre.totalIngresosBillVirt) }}
        </p>
      </div>
    </div>
    ...
    <div class="border-t border-white/10 pt-3">
      ...
      <div
        v-if="cierre.observaciones"
        class="mt-2 p-2 bg-white/5 rounded-md border border-white/10"
      >
        <p class="text-gray-300 text-xs italic">"{{ cierre.observaciones }}"</p>
      </div>
    </div>

    <!-- Hover Effect Overlay -->
    <div
      class="absolute inset-0 bg-gradient-to-r from-primary-400/5 to-secondary-400/5 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none"
    ></div>
  </div>
</template>
