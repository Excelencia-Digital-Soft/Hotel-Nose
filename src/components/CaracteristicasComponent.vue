<template>
  <div class="glass-container p-6 space-y-6">
    ...
    <div class="flex items-center justify-between mb-6">
      <h2
        class="text-2xl font-bold bg-gradient-to-r from-primary-400 via-secondary-400 to-accent-400 bg-clip-text text-transparent"
      >
      ...
      <div class="flex items-center gap-2">
        ...
        <div
          class="w-2 h-2 rounded-full bg-gradient-to-r from-primary-400 to-secondary-400 animate-pulse"
        ></div>
      </div>
    </div>
    ...
    <div v-if="caracteristicas.length > 0" class="space-y-4 z-1">
      <div
        class="grid grid-cols-1 gap-4 max-h-[400px] overflow-y-auto scrollbar-thin scrollbar-thumb-white/20 scrollbar-track-transparent pr-2"
      >
          ...
          >
            <div class="flex items-center justify-between">
              <div class="flex items-center space-x-3">
                ...
                <div class="relative">
                  <div
                    ...
                    class="absolute inset-0 bg-gradient-to-r from-primary-400 to-secondary-400 rounded-lg blur-lg opacity-50 group-hover:opacity-75 transition-opacity"
                  ></div>
                  <img
                    v-if="caracteristica.icono"
                  ...
                  <div
                    v-else
                    class="relative w-10 h-10 rounded-lg bg-gradient-to-br from-primary-400/20 to-secondary-400/20 flex items-center justify-center"
                  >
                    <svg
              ...
              <button
                ...
                class="p-2 rounded-lg bg-red-500/10 hover:bg-red-500/20 border border-red-500/30 hover:border-red-500/50 transition-all duration-200 group/btn"
                title="Eliminar"
              >
                <svg
                  class="w-4 h-4 text-red-400 group-hover/btn:text-red-300"
                  fill="none"
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
...
<script setup>
      ...
      if (newList && newList.length > 0) {
        ...
        try {
          caracteristicas.value =
            await CaracteristicasService.processCaracteristicasWithImages(newList)
        } catch (error) {
  ...
  const agregarCaracteristica = async (nuevaCaracteristica) => {
    ...
    if (nuevaCaracteristica.icono && nuevaCaracteristica.caracteristicaId) {
      try {
        const processedList = await CaracteristicasService.processCaracteristicasWithImages([
          nuevaCaracteristica,
        ])
  ...
  const asignarCaracteristicasAHabitacion = async () => {
    console.log(
      'Debe seleccionar algunas características para la habitación',
      props.idHabitacion,
      caracteristicas.value
    )
    if (!props.idHabitacion) {
      toast.add({
        severity: 'warn',
        summary: 'Datos incompletos',
        detail: 'Se requiere una habitación válida',
        life: 3000,
      })
      return
    }

    // If no characteristics selected, still emit the event to reset the form
    if (caracteristicas.value.length === 0) {
      toast.add({
        severity: 'info',
        summary: 'Sin características',
        detail: 'No hay características seleccionadas para asignar',
        life: 2000,
      })
