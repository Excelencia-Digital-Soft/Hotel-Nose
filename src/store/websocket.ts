...
// @ts-ignore - auth is in JavaScript, will be typed at runtime
  ...
  actions: {
    async connect(): Promise<void> {
      ...
      if (!authStore.isAuthenticated || !institucionID) {
        console.log(
          'WebSocket connection skipped - user not authenticated or no institution selected'
        )
        return
      }
      ...
      try {
        ...
        if (error instanceof Error && error.message.includes('401')) {
          console.error(
            'Authentication failed for WebSocket. Token:',
