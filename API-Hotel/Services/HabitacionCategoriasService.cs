...
namespace hotel.Services
{
    ...
    public class HabitacionCategoriasService : IHabitacionCategoriasService
    {
        ...
        public async Task<ApiResponse<IEnumerable<HabitacionCategoriaDto>>> GetAllByInstitutionAsync(
            int institucionId,
            CancellationToken cancellationToken = default)
        {
            try
            {
                var categorias = await _context.CategoriasHabitaciones
                    .AsNoTracking()
                    ...
                    .Select(c => new HabitacionCategoriaDto
                    {
                        ...
                        Descripcion = null, // No hay campo Descripcion en el modelo actual
                        ...
                        NumeroHabitaciones = c.Habitaciones.Count(h => h.Anulado != true),
                ...
                _logger.LogInformation("Retrieved {Count} room categories for institution {InstitucionId}", 
                    categorias.Count, institucionId);

                return ApiResponse<IEnumerable<HabitacionCategoriaDto>>.Success(categorias);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving room categories for institution {InstitucionId}", institucionId);
            ...
            CancellationToken cancellationToken = default)
        {
            try
            {
                var categoria = await _context.CategoriasHabitaciones
                    .AsNoTracking()
                    .Where(c => c.CategoriaId == id && c.InstitucionID == institucionId && c.Anulado != true)
                    .Select(c => new HabitacionCategoriaDto
                    {
                        CategoriaId = c.CategoriaId,
                        NombreCategoria = c.NombreCategoria ?? string.Empty,
                ...
                if (categoria == null)
                {
                    _logger.LogWarning("Room category with ID {Id} not found for institution {InstitucionId}", id, institucionId);
                ...
                _logger.LogInformation("Retrieved room category {Id}: {Name} for institution {InstitucionId}", 
                    id, categoria.NombreCategoria, institucionId);

                return ApiResponse<HabitacionCategoriaDto>.Success(categoria);
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error retrieving room category with ID {Id} for institution {InstitucionId}", id, institucionId);
                return ApiResponse<HabitacionCategoriaDto>.Failure(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var existingCategoria = await _context.CategoriasHabitaciones
                    .AsNoTracking()
                    .AnyAsync(c => c.InstitucionID == institucionId && 
                ...
                {
                    _logger.LogWarning("Attempted to create room category with duplicate name: {Name} for institution {InstitucionId}", 
                        createDto.NombreCategoria, institucionId);
                    return ApiResponse<HabitacionCategoriaDto>.Failure(
                ...
                _logger.LogInformation("Created room category {Id}: {Name} for institution {InstitucionId}", 
                    categoria.CategoriaId, categoria.NombreCategoria, institucionId);

                return ApiResponse<HabitacionCategoriaDto>.Success(result, "Categoría de habitación creada correctamente");
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync(cancellationToken);
                _logger.LogError(ex, "Error creating room category {Name} for institution {InstitucionId}", 
                    createDto.NombreCategoria, institucionId);
                return ApiResponse<HabitacionCategoriaDto>.Failure(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                var categoria = await _context.CategoriasHabitaciones
                    .FirstOrDefaultAsync(c => c.CategoriaId == id && c.InstitucionID == institucionId && c.Anulado != true, 
                                       cancellationToken);

                if (categoria == null)
                {
                    _logger.LogWarning("Attempted to update non-existent room category with ID {Id} for institution {InstitucionId}", 
                        id, institucionId);
                    return ApiResponse<HabitacionCategoriaDto>.Failure(
                        "Categoría de habitación no encontrada",
                        $"No se encontró la categoría de habitación con ID {id}");
                ...
                var duplicateName = await _context.CategoriasHabitaciones
                    .AsNoTracking()
                ...
                if (duplicateName)
                {
                    _logger.LogWarning("Attempted to update room category {Id} with duplicate name: {Name} for institution {InstitucionId}", 
                        id, updateDto.NombreCategoria, institucionId);
                    return ApiResponse<HabitacionCategoriaDto>.Failure(
                        "Ya existe una categoría con ese nombre",
                ...
                var result = new HabitacionCategoriaDto
                {
                    ...
                    InstitucionId = categoria.InstitucionID,
                    Activo = categoria.Anulado != true,
                    NumeroHabitaciones = await _context.Habitaciones.CountAsync(h => h.CategoriaId == categoria.CategoriaId && h.Anulado != true, cancellationToken),
                    FechaCreacion = categoria.FechaRegistro,
                    FechaModificacion = null
                };

                _logger.LogInformation("Updated room category {Id}: {Name} for institution {InstitucionId}", 
                    id, categoria.NombreCategoria, institucionId);

                return ApiResponse<HabitacionCategoriaDto>.Success(result, "Categoría de habitación actualizada correctamente");
            }
            catch (Exception ex)
            {
                await transaction.RollbackAsync(cancellationToken);
                _logger.LogError(ex, "Error updating room category with ID {Id} for institution {InstitucionId}", id, institucionId);
                return ApiResponse<HabitacionCategoriaDto>.Failure(
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                if (categoria == null)
                {
                    _logger.LogWarning("Attempted to delete non-existent room category with ID {Id} for institution {InstitucionId}", 
                        id, institucionId);
                ...
                var isInUse = await _context.Habitaciones
                    .AsNoTracking()
                    .AnyAsync(h => h.CategoriaId == id && h.Anulado != true, cancellationToken);

                if (isInUse)
                {
                    _logger.LogWarning("Attempted to delete room category {Id} that is in use by rooms for institution {InstitucionId}", 
                        id, institucionId);
                    return ApiResponse.Failure(
                ...
                _logger.LogInformation("Deleted room category {Id}: {Name} for institution {InstitucionId}", 
                    id, categoria.NombreCategoria, institucionId);
            ...
            {
                await transaction.RollbackAsync(cancellationToken);
                _logger.LogError(ex, "Error deleting room category with ID {Id} for institution {InstitucionId}", id, institucionId);
                return ApiResponse.Failure(
