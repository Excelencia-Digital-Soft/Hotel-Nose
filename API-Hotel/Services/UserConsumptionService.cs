...
public class UserConsumptionService : IUserConsumptionService
{
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            var query = _context.Consumo
                .AsNoTracking()
                .Include(c => c.Articulo)
                .Include(c => c.Movimientos)
                    .ThenInclude(m => m.Habitacion)
                .Where(c => c.Movimientos != null && 
            ...
            var consumos = await query
                .Select(c => new UserConsumptionDto
                {
                    ...
                    ArticuloNombre = c.Articulo != null ? c.Articulo.NombreArticulo ?? string.Empty : string.Empty,
                    ...
                    HabitacionNumero = c.Movimientos.Habitacion != null 
            ...
            _logger.LogInformation("Retrieved {Count} consumption records for user {UserId} in institution {InstitucionId}", 
                consumos.Count(), userId, institucionId);

            return ApiResponse<IEnumerable<UserConsumptionDto>>.Success(consumos);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving user consumption for user {UserId} in institution {InstitucionId}", 
                userId, institucionId);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            _logger.LogInformation("Generated consumption summary for user {UserId} in institution {InstitucionId}", 
                userId, institucionId);
        ...
        {
            _logger.LogError(ex, "Error generating consumption summary for user {UserId} in institution {InstitucionId}", 
                userId, institucionId);
        ...
        CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            var consumos = consumosData.Select(c => new UserConsumptionDto
            {
                Id = c.ConsumoId,
            ...
            _logger.LogInformation("Retrieved {Count} consumption records for all users in institution {InstitucionId}", 
                consumos.Count(), institucionId);

            return ApiResponse<IEnumerable<UserConsumptionDto>>.Success(consumos);
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error retrieving all users consumption for institution {InstitucionId}", institucionId);
            return ApiResponse<IEnumerable<UserConsumptionDto>>.Failure(
        ...
        try
        {
            ...
            if (precioUnitario == 0)
            {
                var articulo = await _context.Articulos
                    .AsNoTracking()
                    .FirstOrDefaultAsync(a => a.ArticuloId == createDto.ArticuloId && 
            ...
            var result = await _context.Consumo
                .AsNoTracking()
                .Include(c => c.Articulo)
                .Include(c => c.Movimientos)
                    .ThenInclude(m => m.Habitacion)
            ...
            _logger.LogInformation("Registered consumption {ConsumoId} for user {UserId} in institution {InstitucionId}", 
                consumo.ConsumoId, userId, institucionId);

            return ApiResponse<UserConsumptionDto>.Success(dto, "Consumption registered successfully");
        }
        catch (Exception ex)
        {
            await transaction.RollbackAsync(cancellationToken);
            _logger.LogError(ex, "Error registering consumption for user {UserId} in institution {InstitucionId}", 
                userId, institucionId);
    ...
    public async Task<ApiResponse<IEnumerable<UserConsumptionByServiceDto>>> GetConsumptionByServiceAsync(
        string userId,
        int institucionId,
        DateTime? startDate = null,
        DateTime? endDate = null,
        CancellationToken cancellationToken = default)
    {
        try
        {
                    ...
                    return new UserConsumptionByServiceDto
                    {
                        ...
                        Items = g.Where(c => c.ArticuloId.HasValue)
                            ...
                            .Select(itemGroup => new ServiceItemDetail
                            {
                                ...
                                Amount = itemGroup.Sum(c => (c.Cantidad ?? 0) * (c.PrecioUnitario ?? 0)),
                                LastConsumedDate = itemGroup.Max(c => c.Movimientos?.FechaRegistro ?? DateTime.Now)
                            })
            ...
            _logger.LogInformation("Retrieved consumption by service for user {UserId} in institution {InstitucionId}", 
                userId, institucionId);
        ...
        {
            _logger.LogError(ex, "Error retrieving consumption by service for user {UserId} in institution {InstitucionId}", 
                userId, institucionId);
