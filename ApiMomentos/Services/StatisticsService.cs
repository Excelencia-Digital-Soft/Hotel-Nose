    ...
    public class StatisticsService : IStatisticsService
    {
        ...
        public async Task<List<RoomRankingDto>> GetRoomRankingAsync(DateRangeDto dateRange)
        {
            return await _context.Reservas
                .Where(r => r.FechaReserva >= dateRange.FechaInicio && r.FechaReserva <= dateRange.FechaFin && !r.FechaAnula.HasValue && r.InstitucionID == dateRange.InstitucionID)
                .Join(_context.Habitaciones, r => r.HabitacionId, h => h.HabitacionId, (r, h) => new { r, h })
                .Join(_context.CategoriasHabitaciones, x => x.h.CategoriaId, c => c.CategoriaId, (x, c) => new { x.h, c })
                .GroupBy(x => new { x.h.HabitacionId, x.h.NombreHabitacion, x.c.NombreCategoria })
        ...
        {
            var reservaIngresos = await _context.Reservas
                .Where(r => r.FechaReserva >= dateRange.FechaInicio && r.FechaReserva <= dateRange.FechaFin && !r.FechaAnula.HasValue && r.InstitucionID == dateRange.InstitucionID)
                .Join(_context.Movimientos, r => r.MovimientoId, m => m.MovimientosId, (r, m) => new { r, m })
                .Join(_context.Pagos, x => x.m.PagoId, p => p.PagoId, (x, p) => new { x.r, p })
                .Join(_context.Habitaciones, x => x.r.HabitacionId, h => h.HabitacionId, (x, h) => new { x.r, x.p, h })
                .Join(_context.CategoriasHabitaciones, x => x.h.CategoriaId, c => c.CategoriaId, (x, c) => new
                {
                    x.h.HabitacionId,
                    x.h.NombreHabitacion,
                    c.NombreCategoria,
                    Ingresos = (x.p.MontoEfectivo ?? 0) + (x.p.MontoBillVirt ?? 0) + (x.p.MontoTarjeta ?? 0) - x.p.MontoDescuento + (x.p.Adicional ?? 0)
                })
                .GroupBy(x => new { x.HabitacionId, x.NombreHabitacion, x.NombreCategoria })
                .Select(g => new
                {
                    g.Key.HabitacionId,
            ...
            var consumoIngresos = await _context.Consumo
                .Where(c => c.MovimientosId.HasValue && (c.Anulado == false))
                .Join(_context.Movimientos, c => c.MovimientosId, m => m.MovimientosId, (c, m) => new { c, m })
                .Join(_context.Reservas, x => x.m.VisitaId, r => r.VisitaId, (x, r) => new { x.c, r })
                .Where(x => x.r.FechaReserva >= dateRange.FechaInicio && x.r.FechaReserva <= dateRange.FechaFin && !x.r.FechaAnula.HasValue && x.r.InstitucionID == dateRange.InstitucionID)
                .Join(_context.Habitaciones, x => x.r.HabitacionId, h => h.HabitacionId, (x, h) => new { x.c, h })
                .Join(_context.CategoriasHabitaciones, x => x.h.CategoriaId, c => c.CategoriaId, (x, c) => new
                {
                    x.h.HabitacionId,
                    x.h.NombreHabitacion,
            ...
            return (from r in reservaIngresos
                    join c in consumoIngresos
                    on new { r.HabitacionId, r.NombreHabitacion, r.NombreCategoria } equals new { c.HabitacionId, c.NombreHabitacion, c.NombreCategoria } into consumos
        ...
        public async Task<List<CategoryOccupancyDto>> GetCategoryOccupancyAsync(DateRangeDto dateRange)
        {
            ...
            var totalHabitacionesPorCategoria = await _context.Habitaciones
            ...
            return (from o in ocupacion
                    ...
                    select new CategoryOccupancyDto
                    {
                        ...
                        TasaOcupacion = totalHorasPeriodo > 0 ? (o.TotalHorasOcupadas / (t.TotalHabitaciones * totalHorasPeriodo)) * 100 : 0
                    })
                    .OrderByDescending(x => x.TasaOcupacion)
                    .ToList();
        }
        ...
        public async Task<List<RoomConsumptionDto>> GetRoomConsumptionAsync(DateRangeDto dateRange)
        {
            var consumos = await _context.Consumo
                .Where(c => c.MovimientosId.HasValue && (c.Anulado == false || c.Anulado == null))
                .Join(_context.Movimientos, c => c.MovimientosId, m => m.MovimientosId, (c, m) => new { c, m })
                .Join(_context.Reservas, x => x.m.VisitaId, r => r.VisitaId, (x, r) => new { x.c, r })
                .Where(x => x.r.FechaReserva >= dateRange.FechaInicio && x.r.FechaReserva <= dateRange.FechaFin && !x.r.FechaAnula.HasValue && x.r.InstitucionID == dateRange.InstitucionID)
                .Join(_context.Habitaciones, x => x.r.HabitacionId, h => h.HabitacionId, (x, h) => new { x.c, h })
                .Join(_context.CategoriasHabitaciones, x => x.h.CategoriaId, c => c.CategoriaId, (x, c) => new { Consumo = x.c, Habitacion = x.h, Categoria = c })
                .Join(_context.Articulos, x => x.Consumo.ArticuloId, a => a.ArticuloId, (x, a) => new
                {
                    ...
                    a.NombreArticulo,
                    x.Consumo.Cantidad,
                    PrecioTotal = (x.Consumo.PrecioUnitario ?? 0) * (x.Consumo.Cantidad ?? 0)
                })
                .ToListAsync();
